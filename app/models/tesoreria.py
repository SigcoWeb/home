...
class SgcCaja(Base):
    ...
    moneda = Column(String(3), nullable=False) # PEN, USD
...
class SgcMovimientoTesoreria(Base):
    ...
    tipo_movimiento = Column(String(20), nullable=False) # INGRESO, EGRESO, TRANSFERENCIA
    ...
    referencia = Column(String(100), nullable=True) # Nro Operacion, Cheque, etc.
    
    # Relacion con otros modulos (opcional por ahora)
    # id_venta = ...
    # id_compra = ...

    # Transferencias
    es_transferencia = Column(Boolean, default=False)
    id_movimiento_relacionado = Column(Integer, nullable=True) # Para vincular ingreso con egreso
